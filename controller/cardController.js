...
const CoinRate = require('../models/CardRate');
...
const cardUserGetController = async (req, res, next) => {


    try {
        const coinRating = await CoinRate.findOne({ role: 'admin' });
        res.status(200).json({
            cardRating: coinRating.rating
        })
    }
    catch (e) {
...
const cardUserPostController = async (req, res, next) => {
    let { catchAmount, cardCol, cardValue } = req.body;
    ...
    //type casting 
    var catchNumber = parseFloat(1, 10);
    ...
    try {
        ...
        let rating = parseFloat(coinRate.rating, 10);
...
module.exports = { cardUserPostController, cardUserGetController }
