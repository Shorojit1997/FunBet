...
const CardPlay = () => {
    ...
    const { cardInfo,flashMessage} = useSelector(state => state.adminCard, shallowEqual);
    ...
    const callMethod = useCallback(() => {
        if (cardInfo)
            setLudo({
                ...
                winningPossibility: cardInfo.winningPossibility,
                minimumBetsAmount: cardInfo.minimumBetsAmount
            })
    }, [cardInfo])
    ...
    const changeHandeler = () => {
        axios.post('/api/admin/games/card', ludo)
            .then(info => {
                if (info.data) {
                    dispatch({
                        type: Types.ADMIN_CARD_GET, payload: {
                            flashMessage: info.data.flashMessage,
                            cardInfo: info.data.cardrate
                        }
                    })
                }
            })
        ...
        <div className='d-flex justify-content-center  w-100'>
            <div className='card card-body m-2 p-1' style={{ maxWidth: '700px', minWidth: '350px' }} >
                ...
                {/* ludo setting  */}
                {
                        ...
                        <>
                            <div className='p-2 align-left font-weight-bold'>Card settings:</div>
