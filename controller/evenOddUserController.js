...
const EvenOddRate = require('../models/EvenOddRate');
...
const evenOddUserGetController = async (req, res, next) => {


    try {
        const evenOddRating = await EvenOddRate.findOne({ role: 'admin' });
        res.status(200).json({
            evenOddRating: evenOddRating.rating
        })
    }
    catch (e) {
...
const evenOddUserPostController = async (req, res, next) => {
    ...
    try {
        ...
        const wheelRate = await EvenOddRate.findOne({ role: 'admin' });
        ...
        const gamebets = new GameBets({
            ...
            userId: req.user._id,
            Stake: parseInt(catchNumber) % 2 == 1 ? 'Odd' : 'Even',
            R_stake: parseInt(randNumber) % 2 == 1 ? 'Odd' : 'Even',
...
module.exports = { evenOddUserGetController, evenOddUserPostController }
